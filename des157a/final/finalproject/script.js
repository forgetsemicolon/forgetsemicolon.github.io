(function() {
    ...
    function displayInfo() {
        ...
        const myText = "Hello! Welcome to squid game!";

        inst.innerHTML = myText;

        inst.innerHTML += "<br><br>RULES: There are two players. The player whose turn it is rolls the dice. The total of the roll is added to the current player's score, unless either die comes up as a 'one'. If this happens, this player's turn is over, and it is the other player’s turn. After each roll, the current player can either roll again, (assuming a 'one' was not rolled) or if the current player feels that luck is running thin, they can pass to the other player.";
    ...
    function throwDice() {
        ...
        if (gameData.index == 0) {
            squid2.style.opacity = '0.5';
            squid1.style.opacity = '1';
        }
        else {
            squid1.style.opacity = '0.5';
            squid2.style.opacity = '1';
        }
    ...
    function checkWinningCondition () {
        if (gameData.score[gameData.index] > gameData.gameEnd) {
            ...
            gameControl.innerHTML += '<button id="start">Start a New Game?</button>'
